...
namespace Newton
{
    public partial class MainForm : Form
    {
        ...
        private bool spikeFilterEnabled = true;
        private double spikeThresholdN = 2.5;
        private bool haveLastGoodA = false;
        private bool haveLastGoodB = false;
        private double lastGoodA = 0;
        private double lastGoodB = 0;
        ...
        private volatile bool isReadingA = false;
        private volatile bool isReadingB = false;
                   ...
                   (slot == "B" && isReadingB))
            {
                try
                {
                    ...
                    if (TryParseForceLine(line, out ForceSample sample))
                    {
                        ...
                        // opcionális spike szűrés már az előjeles értékre
                        displayForce = ApplySpikeFilter(slot, displayForce);
                        ...
                        BeginInvoke(new Action(() =>
                        {
                            if (slot == "A")
                            {
                                if (chkPhoneMode.Checked)
                                    lblStatusA.Text =
                                        $"A: Ay={displayForce:F3} m/s²";
                                else
                                    lblStatusA.Text =
                                        $"A: F={displayForce:F3} N";
                            }
                            else
                            {
                                lblStatusB.Text =
        ...
        private double ApplySpikeFilter(string slot, double force)
        {
            ...
            if (slot == "A")
            {
                ...
                if (Math.Abs(force - lastGoodA) > spikeThresholdN)
                    return lastGoodA;
            ...
            else
            {
                ...
                if (Math.Abs(force - lastGoodB) > spikeThresholdN)
                    return lastGoodB;
...
EventArgs e)
        ...
        private void chk_spike_CheckedChanged(object sender, EventArgs e)
        {
            spikeFilterEnabled = chk_spike.Checked;
        }

        private void txt_spike_TextChanged(object sender, EventArgs e)
        {
            if (double.TryParse(txt_spike.Text.Replace(",", "."),System.Globalization.NumberStyles.Float,
